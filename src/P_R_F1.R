...
get_R_P_F <- function(score_data,genelist,data_mut,top_n){
  ...
  for(k in 1:length(samples)){
    ...
    if(n_com_genes!=0){
      ...
      } else {  P_R_F1[[k]]=cbind(P_R_F1_sample,matrix(rep(P_R_F1_sample[,ncol(P_R_F1_sample)],top_n-ncol(P_R_F1_sample)),byrow = F,nrow = 3)) }
...
get_top_n <- function(methods_nums,score_data,methods_names,genelist,data_mut,genelist_name){
  ...
  plot(plot_data[[1]][1,],type = "o",ylim = c(0,1),xlim=c(0,20),pch=16,col=mycolors[1],cex=0.6,frame.plot = F,xlab="Top N genes",ylab="Average precision")
  for ( i in (1:methods_nums)){
    points(plot_data[[i]][1,],type = "o",pch=15+i,col=mycolors[i],lty=i,cex=0.6)
  }
  legend(8,1,methods_names,col=mycolors[1:methods_nums],text.col=mycolors[1:methods_nums],lty=c(1:methods_nums),pch=c(16:(15+methods_nums)),bty="n",cex = 0.8)  
  
  plot(plot_data[[1]][2,],type = "o",ylim = c(0,1),xlim=c(0,20),pch=16,col=mycolors[1],cex=0.6,frame.plot = F,xlab="Top N genes",ylab="Average recall",main=genelist_name)
  for ( i in (1:methods_nums)){
    points(plot_data[[i]][2,],type = "o",pch=15+i,col=mycolors[i],lty=i,cex=0.6)
  }
  
  plot(plot_data[[1]][3,],type = "o",ylim = c(0,0.5),xlim=c(0,20),pch=16,col=mycolors[1],cex=0.6,frame.plot = F,xlab="Top N genes",ylab="Average F1")
  for ( i in (1:methods_nums)){
